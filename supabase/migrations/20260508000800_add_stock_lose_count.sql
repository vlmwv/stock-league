-- 1. stocks 테이블에 실패 수(lose_count) 컬럼 추가
ALTER TABLE public.stocks ADD COLUMN IF NOT EXISTS lose_count INTEGER DEFAULT 0;

-- 2. 예측 성공(predictions) 트리거 함수 수정
CREATE OR REPLACE FUNCTION public.handle_prediction_win_stats()
RETURNS TRIGGER AS $$
DECLARE
    v_stock_id BIGINT;
    v_win_count INTEGER;
    v_lose_count INTEGER;
...
BEGIN
    ...
    -- 2. 최종 win_rate 재계산 (처리 완료된 항목 기준)
    SELECT win_count, lose_count, 
           (SELECT count(*) FROM public.predictions WHERE stock_id = v_stock_id AND result IN ('win', 'lose', 'draw')) as processed_count
    INTO v_win_count, v_lose_count, v_processed_count
    FROM public.stocks WHERE id = v_stock_id;

    IF v_processed_count > 0 THEN
        UPDATE public.stocks 
        SET win_rate = ROUND((v_win_count::NUMERIC / v_processed_count::NUMERIC) * 100, 1)
        WHERE id = v_stock_id;
    ELSE
...
SET 
    ...
    lose_count = (SELECT count(*) FROM public.predictions p WHERE p.stock_id = s.id AND p.result = 'lose');

-- win_rate 일괄 계산 (완료된 예측 기준)
UPDATE public.stocks s
SET win_rate = CASE 
    WHEN (SELECT count(*) FROM public.predictions p WHERE p.stock_id = s.id AND p.result IN ('win', 'lose', 'draw')) > 0 
    THEN ROUND((win_count::NUMERIC / (SELECT count(*) FROM public.predictions p WHERE p.stock_id = s.id AND p.result IN ('win', 'lose', 'draw'))::NUMERIC) * 100, 1)
    ELSE 0 
END;
