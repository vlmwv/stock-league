...
| :--- | :--- | :--- | :--- |
| `fetch-market-news-periodically` | 매시간 정각 (주간 8종목 / 야간 2종목 요약) | 최신 뉴스 수집 및 AI 요약 | `SELECT * FROM news ORDER BY published_at DESC LIMIT 10;` |
| `update-krx-top-100` | 매일 16:00 | 시가총액 상위 100종목 갱신 | `SELECT * FROM stocks WHERE market_cap_rank <= 100;` |
