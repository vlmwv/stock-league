...
```
┌─────────────── 프론트엔드 (Nuxt 4 / Vue 3) ───────────────┐
│  pages/ (17개)  ─  components/ (16개)  ─  middleware (2)   │
│                         ↓                                   │
│  composables/ useStock(파사드 96줄) + 도메인 12개 · useScenario │
│  utils/{stock, stockHistory}.ts (순수함수)                      │
└────────────┬──────────────────────────┬────────────────────┘
             │ 직접 쿼리(RLS)            │ 교차유저 집계만
             ↓                          ↓
   ┌──────────────────┐      ┌─────────────────────────┐
   │  Supabase Client │      │ server/api/** (Nitro 14) │
...
### 2.1 프론트엔드 데이터 계층
- **`useStock.ts`는 도메인 컴포저블을 합쳐 내보내는 96줄 파사드**다(착수 시 2089줄 단일 거대 컴포저블 → 12개 도메인 컴포저블로 분리 완료, `docs/refactor-usestock-plan.md`). 파사드는 도메인을 가로지르는 오케스트레이션(`refreshAll`·`watch(user)`·`allPredicted`)과 반환 표면 조합만 담당하고, 소비자 18곳은 여전히 `useStock()`만 호출한다(반환 키 불변).
  ...
  | `useUtils` | 날짜/시간 포맷터(34줄, `admin` 페이지에서만 사용) |
...
- `useScenario.ts`(628줄)는 10개 시나리오를 **하드코딩**(약 1440개 캔들 데이터)으로 메모리에 로드 — 잔여 백로그(DB 이관 예정).
...
| # | 상태 | 리스크 | 위치 | 영향 |
|---|------|--------|------|------|
| 🔴 | ✅ | **`useStock.ts` 단일 거대 파일** — 영역별 컴포저블로 분리 완료(2089→96줄 파사드 + 도메인 12개). `docs/refactor-usestock-plan.md` 참고 | composables | 유지보수성·테스트 불가·머지 충돌 |
...
### ✅ 완료
1. **(고) `useStock.ts` 분할** — `useDailyStocks`, `usePredictions`, `useWishlist`, `useRankings`, `useNews`, `useUserProfile`, `useAiHistory` 등 도메인 12개로 분리(2089→96줄 파사드).
