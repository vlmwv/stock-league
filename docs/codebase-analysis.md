...
- `useScenario.ts`(152줄)는 10개 시나리오를 **DB(`public.scenarios`)에서 `useAsyncData`로 조회**한다. 과거 하드코딩(약 1440 캔들, 절차적 생성) 원본은 `scripts/scenario-seed-data.ts`로 이동해 앱 번들에서 분리했고, `scripts/seed_scenarios.ts`로 시드한다.
