...
- `useStock.ts` 현재 **96줄** (착수 시 2089줄). 분리 완료(1~8단계). 이후는 선택적 소비자 마이그레이션(9단계)만 남음.
...
- ✅ **8단계 완료** — `notifications` computed를 `useNews`로 이관(daily `recommended` 주입). 파사드는 오케스트레이션(`refreshAll`·`watch(user)`·`allPredicted`)과 반환 표면 조합만 남기고 **96줄**로 축소(착수 시 2089줄). 반환 키 전체 1:1 동일 → 소비자 영향 0.
...
8. ✅ **파사드 정리** — `notifications`를 `useNews`로 이관하고 `refreshAll`·`watch(user)`·`allPredicted` 조합만 남겨 `useStock`을 96줄로 축소.
