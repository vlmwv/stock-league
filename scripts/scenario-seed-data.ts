// 시드 전용 시나리오 원본 데이터 (절차적 캔들 생성기 포함).
// 과거 app/composables/useScenario.ts에 하드코딩돼 있던 것을 앱 번들에서 분리하기 위해
// scripts/ 아래로 이동했다. scripts/seed_scenarios.ts가 생성기를 실행해 만든 결과를
// public.scenarios 테이블에 구워 넣고, 런타임 useScenario는 DB에서 조회한다.
// 이 파일은 앱에서 import하지 않으므로 클라이언트 번들에 포함되지 않는다.
