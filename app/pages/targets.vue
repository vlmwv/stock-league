    ...
    <main class="max-w-md mx-auto px-6 py-8">
      <header class="mb-8">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 mb-4">
          ...
          <span class="text-[10px] font-black text-emerald-400 uppercase tracking-widest">AI Target Prices</span>
        </div>
        <h2 class="text-3xl font-black text-slate-100 tracking-tight leading-tight">
          AI 선정 <span class="text-emerald-400">목표가 리스트</span>
        </h2>
        <p class="text-sm text-slate-500 mt-3 font-medium leading-relaxed">
          AI가 분석한 중장기 목표가와 목표달성 예상일 정보를 한눈에 확인하세요.
        </p>
      </header>
      ...
      <section v-else-if="!targetedStocks || targetedStocks.length === 0" class="flex flex-col items-center justify-center py-20 px-4 text-center">
        ...
        <h3 class="text-xl font-black text-slate-100 mb-2">목표가 데이터가 없습니다</h3>
        ...
        >
            ...
            <div class="flex gap-4">
              ...
              <div>
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span class="text-[9px] font-black text-slate-600 bg-slate-800/50 px-1.5 py-0.5 rounded border border-white/5 uppercase tracking-widest">{{ stock.game_date }} 추천</span>
                </div>
                <h4 class="text-xl font-black text-slate-100 group-hover:text-emerald-400 transition-colors">{{ stock.name }}</h4>
              </div>
            </div>
            <div class="text-right">
              <div class="text-sm font-black text-slate-400">현재 {{ stock.last_price.toLocaleString() }}원</div>
              <div 
          ...
          <div class="relative bg-emerald-500/5 rounded-2xl border border-emerald-500/10 p-5 mb-6 overflow-hidden">
            ...
            <div class="grid grid-cols-2 gap-4 relative z-10">
              <div>
                <p class="text-[10px] font-black text-emerald-500/70 uppercase tracking-widest mb-1">AI 목표가</p>
              ...
              <div class="text-right">
                <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">목표기준일</p>
                <p class="text-base font-black text-slate-200">{{ stock.target_date }}</p>
              </div>
            </div>

            <!-- Potential Upside -->
            <div class="mt-4 pt-4 border-t border-emerald-500/10 flex items-center justify-between">
              <span class="text-[10px] font-bold text-slate-500">예상 수익률</span>
          ...
          <div class="space-y-3">
            <div class="flex items-center gap-2">
              <UIcon name="i-heroicons-chat-bubble-bottom-center-text" class="w-4 h-4 text-slate-500" />
              <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">AI 분석 요약</span>
            </div>
            <p class="text-xs text-slate-400 leading-relaxed italic line-clamp-2 bg-slate-800/20 p-3 rounded-xl border border-white/5">
              "{{ stock.summary }}"
            </p>
          </div>
        </div>
...
useHead({
  title: 'AI 목표가 리스트 - 스탁 리그',
  meta: [
    { name: 'description', content: 'AI가 분석한 종목별 목표가와 목표기준일 정보를 확인하세요.' }
  ]
})
</script>
