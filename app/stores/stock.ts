...
export const useStockStore = defineStore('stock', {
  state: () => ({
    ...
    myPredictions: [] as { stockId: number, prediction: 'up' | 'down' }[]
  }),
  actions: {
    predict(stockId: number, prediction: 'up' | 'down') {
      const index = this.myPredictions.findIndex(p => p.stockId === stockId)
      if (index > -1) {
        this.myPredictions[index].prediction = prediction
      } else {
        this.myPredictions.push({ stockId, prediction })
      }
    }
  }
})
